<template>
	...
	<view class="card">
		<!-- 顶部主图 -->
		<view class="avator">
			<image :src="data.resList[0].resUrl" mode="aspectFill" class="master"></image>
		...
		<!--  -->
		<view class="conten">
			<view class="headline">{{data.activityTitle}}</view>
			<!-- 活动卡无优惠券显示 -->
			<!-- <view class="here flex" v-if="data.cutPrice<=0">
				<image src="../../static/logo.png" mode="" class="imgH"></image>
				<view class="by">by here</view>
			</view> -->
			<!-- 活动卡有优惠券显示 -->
			<view class="here flex">
		...
		<view class="html" style="padding-right:33upx;padding-left: 33upx;margin-top: 30rpx;width: auto; height: auto;">
	...
	<!-- 	<view class="html" style="padding-right:22upx;padding-left: 22upx;margin-top: 50rpx; box-sizing: border-box;">
		...
		<uni-popup ref="popup" type="center" :custom="true" :maskClick="false">
			...
			<view class="content">
				...
				<view class="content_">还没拥有Here? AppStore搜索“here” 即刻发现身边的精彩吧</view>
			...
			getDetails(){
				...
				this.$http.post(this.$api.findCardActivityInfo, {
					...
					if (result.data.code == 0) {
						...
						let infos = this.data.activityContent
						if(infos.includes('-->')){
						...
						this.$http.post(this.$api.findActivityJoinUser, {
							data: aa,
							header: {
								'content-type': 'application/json;charset=UTF-8',
								'token': this.token
